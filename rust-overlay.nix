...
let
  ...
  parseRustToolchain = file:
    ...
    if file == null then
      ...
      in if channel != null then
        ...
        let toolchain = (fromTOML (readFile file)).toolchain;
        in (parseRustChannel toolchain.channel) // {
  ...
  fromManifestFile = manifest:
    { stdenv, fetchurl, patchelf, targets, extensions }:
    let
  ...
  fromManifest = sha256: manifest:
    ...
    in fromManifestFile manifestFile {
      inherit stdenv fetchurl patchelf;
      inherit (manifest) extensions targets;
    };
